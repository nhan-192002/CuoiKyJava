...
//chat giua nguoi dung voi nhau
public class ClientCommunicateThread extends Thread {

	Client thisClient;
//Trong phương thức ClientCommunicateThread(Socket clientSocket), 
//clientSocket.getPort() được gọi để lấy cổng của client. 
//Sau đó, thông tin này được gán cho thuộc tính port của đối tượng Client (thisClient.port = clientSocket.getPort();)
	...
	public void run() {
		try {
			while (true) {
				String header = thisClient.receiver.readLine(); //đọc một dòng văn bản.
				if (header == null)
				...
				case "new login": {
					...
					String clientUsername = thisClient.receiver.readLine();  //đọc một dòng văn bản.
					...
					if (!userNameExisted) {
						...
						//ghi một phần của một chuỗi.
						thisClient.sender.write("login success");
						// thêm một dòng mới với dấu xuống dòng
				...
				case "text to room": {
					...
					char c;
					do {
						c = (char) thisClient.receiver.read();  //đọc ký tự vật duy nhất
						//Kí tự '\0' hay NULL có mã ASII là 0 dùng:
//						-Làm kí tự kết thúc chuỗi
//						-Giá trị của con trỏ chưa cấp phát hoặc chưa trỏ tới địa chỉ trên bộ nhớ
//
//
						if (c != '\0')
							content += c;
...
//				tệp tin được gửi từ client tới room.
				case "file to room": {
					int roomID = Integer.parseInt(thisClient.receiver.readLine());
...
//				}
			...
			if (!Main.socketController.s.isClosed() && thisClient.userName != null) {
				...
				//Xóa người dùng vừa thoát khỏi danh sách client kết nối.
